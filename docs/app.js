...
FIX (2026-09-24): Import `spaces` BEFORE any CUDA/torch-related packages
so HF Spaces ZeroGPU does not raise:
  RuntimeError: CUDA has been initialized before importing the `spaces` package.

UPDATE (2026-09-25): Third output = JSON for pure PWA clients (GitHub/CF Pages).
"""

# ── CRITICAL: spaces MUST be the first import that can touch CUDA ──────────
try:
...
async def check_claim(claim: str):
    ...
    if len(claim) < 8:
        empty = {
            ...
            "summary": "Please write a longer claim (at least 8 characters).",
            "confidence_score": 0,
            "detailed_breakdown": [],
            "trusted_sources": [],
            "disclaimer": "Educational use only. Not medical advice.",
        }
        return "Please write a longer claim (at least 8 characters).", "", json.dumps(empty)
    ...
    if isinstance(breakdown, str):
        ...
        result["detailed_breakdown"] = breakdown
    sources = result.get("trusted_sources", []) or []
...
""" + "\n".join(f"• {b}" for b in breakdown)
    ...
    return html, plain, json.dumps(result)
...
with gr.Blocks(title="Health Buddy") as demo:
    ...
    # Hidden JSON payload for pure-PWA / external clients (3rd output of /check_claim)
    result_json = gr.Textbox(visible=False, label="json")

    check_btn.click(
        fn=check_claim,
        inputs=claim_box,
        outputs=[result_html, result_text, result_json],
        api_name="check_claim",
    )
    clear_btn.click(
        fn=lambda: ("", "", "", ""),
        inputs=None,
        outputs=[claim_box, result_html, result_text, result_json],
    )
